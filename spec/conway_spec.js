...
/***/ function(module, exports, __webpack_require__) {
	...
	describe("conway-logic", function(){
	  ...
	  describe("read", function(){
	    ...
	    return thatIt("living cells turn into coordinates with fix-point in the top-left corner", function(){
	  ...
	  describe("print", function(){
	    thatIt("turns a single cell (any coordinates) into an X", function(){
	      return [[0, 0], [0, 1], [1, 0], [12, 4], [4, 12]].forEach(function(coords){
	        return expect(conway.print([coords])).toEqual("X");
	      });
	    });
	    thatIt("turns a dead cells between living cells into a -", function(){
	      return [[[[0, 0], [0, 2]], "X-X"]].forEach(function(arg$){
	        var coords, str;
	  ...
	  describe("boundaries", function(){
	    thatIt("returns the top-left and bottom-right coordinates given an array of coordinates", function(){
	  ...
	  describe("expand-boundaries", function(){
	    return thatIt("returns a pair of boundary coordinates that has been expanded by one", function(){
	  ...
	  describe("coordinates-equals", function(){
	    ...
	    return thatIt("returns false if the coordinates are not equal", function(){
	      return expect(conway.coordinatesEquals([0, 0], [1, 0])).toEqual(false);
	    });
	  });
	  describe("coordinates-in-list", function(){
	    thatIt("returns true if the coordinates are in a list of coordinates", function(){
	      return expect(conway.coordinatesInList([[0, 0], [0, 1], [0, 2]], [0, 0])).toEqual(true);
	    });
	    return thatIt("returns false if the coordinates are not in a list of coordinates", function(){
	  ...
	  describe("count-living-neighbours", function(){
	    return thatIt("returns the number of neighbours that are present in the list of living-cells passed in", function(){
	      return expect(conway.countLivingNeighbours([[0, 0]], [1, 1])).toEqual(1);
	    });
	  });
	  describe("will-cell-live", function(){
	    thatIt("makes any living cell with fewer than two live neighbours die, as if caused by under-population", function(){
	      return [0, 1].forEach(function(count){
	        return expect(conway.willCellLive({
	          living: true,
	          numberOfNeighbours: count
	        })).toBe(false);
	      });
	    });
	...
	//# sourceMappingURL=C:\Users\Florian\OneDrive\Dokumente\GitHub\conway-game-of-life\node_modules\livescript-loader\index.js!C:\Users\Florian\OneDrive\Dokumente\GitHub\conway-game-of-life\spec\conway-spec.ls.map
...
/***/ function(module, exports, __webpack_require__) {
	...
	print = function(livingCellsCoordinates){
	  return join("\n")(
	  map(join(""))(
	  map(map(function(coord){
	    if (coordinatesInList(livingCellsCoordinates, coord)) {
	      return 'X';
	    } else {
	      return '-';
	    }
	...
	coordinatesEquals = curry(function(arg$, arg1$){
	  var x1, y1, x2, y2;
	  x1 = arg$[0], y1 = arg$[1];
	  x2 = arg1$[0], y2 = arg1$[1];
	  return x1 === x2 && y1 === y2;
	});
	coordinatesInList = curry(function(list, coordinates){
	  return !!find(coordinatesEquals(coordinates), list);
	});
	boundaries = function(livingCellsCoordinates){
	  var xses, yses;
	  xses = map(at(0), livingCellsCoordinates);
	  yses = map(at(1), livingCellsCoordinates);
	  return [[minimum(xses), minimum(yses)], [maximum(xses), maximum(yses)]];
	};
	expandBoundaries = function(arg$){
	  var ref$, xMin, yMin, xMax, yMax;
	  ref$ = arg$[0], xMin = ref$[0], yMin = ref$[1], ref$ = arg$[1], xMax = ref$[0], yMax = ref$[1];
	  return [[xMin - 1, yMin - 1], [xMax + 1, yMax + 1]];
	};
	coordinatesBeetween = function(arg$){
	...
	neighbourCoordinates = function(arg$){
	  var x, y;
	  x = arg$[0], y = arg$[1];
	...
	countLivingNeighbours = function(livingCellsCoordinates, coordinates){
	  ...
	  filter(coordinatesInList(livingCellsCoordinates))(
	  neighbourCoordinates(
	  coordinates)));
	};
	willCellLive = function(arg$){
	  var living, numberOfNeighbours;
	  living = arg$.living, numberOfNeighbours = arg$.numberOfNeighbours;
	  switch (living) {
	  case true:
	...
	tick = function(livingCellsCoordinates){
	  return filter(function(coords){
	    return willCellLive({
	      living: coordinatesInList(livingCellsCoordinates, coords),
	...
	module.exports = {
	  ...
	  coordinatesBeetween: coordinatesBeetween,
